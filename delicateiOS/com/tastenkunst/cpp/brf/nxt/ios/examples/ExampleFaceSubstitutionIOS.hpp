#ifndef __brf__ios__ExampleFaceSubstitutionIOS__
#define __brf__ios__ExampleFaceSubstitutionIOS__
...
class ExampleFaceSubstitutionIOS : public ExampleBase {

	public: brf::Rectangle _faceDetectionROI;
	
	public: ExampleFaceSubstitutionIOS(int width, int height) :
			ExampleBase(width, height, true),
			_faceDetectionROI() {
	}
	
	public: ~ExampleFaceSubstitutionIOS() {
	}
	
	public: void onReadyBRF() {
		brf::trace("ExampleFaceSubstitutionIOS::onReadyBRF");
		...
		// face Substitution settings: min and max faceShape width
		
		_brfManager.setFaceTrackingVars(80, 500);
		
		// and candide Substitution disabled in this example.
		
		_brfManager.candideEnabled(false);
		_brfManager.candideActionUnitsEnabled(false);
		
		// we want it all, so set the mode to face Substitution, not just face detection or point Substitution
...
//			DrawingUtils::drawRect(context, faceShape.bounds, false, 1, 0x00ff00, 1);
			...
			//updateVertices(faceShape.faceShapeVertices, faceShapeVertices3);
		}
	}
};

#endif /* __brf__ios__ExampleFaceSubstitutionIOS__ */